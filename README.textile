...
h2. Redis Health Check (optional)

Pulse includes an optional Redis health check. To add the Redis health check, create a new instance of @Pulse::RedisHealthCheck@ with your Redis client connection.
...
<pre>
  <code>
    require 'rails-pulse'
    Pulse.health_checks << Pulse::RedisHealthCheck.new($redis)
  </code>
</pre>

h2. Custom Health Checks

You can add custom health checks to Pulse. Pulse will call @#healthy?@ on each health check. If any check returns @false@, Pulse will trigger an error response.

So if you had a custom health check:

<pre>
  <code>
    class WebScaleDBHealthCheck
      def healthy?
        true # always ;)
      end
    end
...
<pre>
  <code>
    require 'rails-pulse'
    Pulse.health_checks << WebScaleDBHealthCheck.new
  </code>
</pre>
